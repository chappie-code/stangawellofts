...
//-------------------------------------------------------
	...
	initialize: function(slideshow, props) {
		this.props = Object.extend({
			...
			hu: '/',
			images: [],
			navigation: false,//	  navigation    - Optional navigation controls. The navigation appears as anchors (A) within an unordered list (UL LI). If called with the 
			//optional 'fast' keyword (eg. 'arrows fast'), the slideshow will not wait until the current transition completes, but update the slide change instantly
//          arrows: //Only previous and next controls
//          arrows+: Previous, next and controls to jump to any image
			pan: 50,//	  pan           - Optional value to customize slideshow panning movement. Acceptable values are whole integers between 1 and 100 or the keyword 'rand'
			//which will generate a random value for each slide

			resize: true,//Image resizing - especially zooming - doesn't look so great in the current version of Windows. To use Slideshow without resizing images, 
			//intialize the class with resize: false and be sure your images are larger than the slideshow's display area. Note, this only works with fade, pan, push and 
			//wipe type slideshows - zoom or combo type slideshows 
			thumbnailre: [/\./, 't.'],
			transition: Fx.Transitions.sineInOut,
			type: 'fade',
			height: false,//	  height        - Optional height value for the slideshow as a whole integer. If a height value is not given the height of the default image will be used
			width: false,
			zoom: 100
		}, props || {});
